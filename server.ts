...
async function getBlocksData(blockId: string) {
  ...
  return blocks.map((block: any) => {
    switch (block.type) {
      case "paragraph":
        return { type: "p", text: extractRichText(block.paragraph.rich_text) };
      case "heading_1":
        return { type: "h1", text: extractRichText(block.heading_1.rich_text) };
      case "heading_2":
        return { type: "h2", text: extractRichText(block.heading_2.rich_text) };
      case "heading_3":
        return { type: "h3", text: extractRichText(block.heading_3.rich_text) };
      case "bulleted_list_item":
        return { type: "li", text: extractRichText(block.bulleted_list_item.rich_text) };
      case "divider":
        return { type: "hr" };
      case "video":
        return { type: "video", url: block.video.file?.url || block.video.external?.url };
      case "image":
        return { type: "image", url: block.image.file?.url || block.image.external?.url };
      default:
